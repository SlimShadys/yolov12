...
def piou(box1, box2, xywh=True, PIoU=False, PIoU2=False, Lambda=1.3, eps=1e-7):
    ...
    if PIoU:
        ...
        q = torch.exp(-P)
        x = q * Lambda
        L_v2 = 3 * x * torch.exp(-(x**2)) * L_v1
        return iou - L_v2  # PIoU2
    else:
        raise ValueError("PIoU or PIoU2 must be True in Loss computation. Set PIoU=True or PIoU2=True to calculate the respective IoU variant.")
